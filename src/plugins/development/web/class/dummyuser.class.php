...
var $_role_table = '';
	...
	*/
	...
	function set_user_language($name, $lang) {
		$db=openqrm_get_db_connection();
		$sql = "update ? set user_lang=? where user_name=?";
		$rs = $db->Execute($sql,[$this->_user_table,$lang,$name]);
	}
	...
	function set_wizard($username, $wizardname, $step, $id) {
		$db=openqrm_get_db_connection();
		$sql = "update ? set user_wizard_name=?,  user_wizard_step=?,user_wizard_id=? where user_name=?";
		$rs = $db->Execute($sql,[$this->_user_table,$wizardname,$step,$id,$username]);
		return $rs;
	}
